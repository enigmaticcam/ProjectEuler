...
namespace Euler_Logic.Problems.AdventOfCode.Y2017 {
    public class Problem12 : ProblemBase {
        ...
        private string Answer1(List<string> input) {
            ...
            foreach (string line in input) {
                ...
                foreach (int number in chain) {
                    AddToChain(number, value, entries);
                }
            }

            // Count up, starting at 0
            int count = 1;
            Entry entry = entries[0];
            while (entry.Upper != null) {
                count++;
                entry = entry.Upper;
            }
        ...
        private void AddToChain(int value, int chain, Dictionary<int, Entry> entries) {
            Entry entryToAdd = GetEntry(value, entries);
            Entry entryToSearch = GetEntry(chain, entries);
            if (value > chain) {
                do {
                    if (entryToSearch.Upper == null) {
                        entryToSearch.Upper = entryToAdd;
                        entryToAdd.Lower = entryToSearch;
                        break;
                    } else if (entryToSearch.Upper.Value > value) {
                        entryToSearch.Upper.Lower = entryToAdd;
                        entryToAdd.Upper = entryToSearch.Upper;
                        entryToAdd.Lower = entryToSearch;
                        entryToSearch.Upper = entryToAdd;
                        break;
                    } else if (entryToSearch.Upper.Value == value) {
                    ...
                    entryToSearch = entryToSearch.Upper;
                } while (true);
            } else if (value < chain) {
                do {
                    if (entryToSearch.Lower == null) {
                        entryToSearch.Lower = entryToAdd;
                        entryToAdd.Upper = entryToSearch;
                        break;
                    } else if (entryToSearch.Lower.Value < value) {
                        entryToSearch.Lower.Upper = entryToAdd;
                        entryToAdd.Upper = entryToSearch;
                        entryToAdd.Lower = entryToSearch.Lower;
                        entryToSearch.Lower = entryToAdd;
                        break;
                    } else if (entryToSearch.Lower.Value == value) {
                        break;
                    }
                    entryToSearch = entryToSearch.Lower;
                } while (true);
            }
        }

        private Entry GetEntry(int value, Dictionary<int, Entry> entries) {
